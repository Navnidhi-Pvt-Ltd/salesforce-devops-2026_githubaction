...
sf project deploy start --test-level RunSpecifiedTests --tests OnboardingTriggerHandlerTest OnboardingReminderBatchTest  --source-dir delta/force-app/main/default --target-org UAT_INT_ORG --wait 45 --dry-run --json 2>&1 | tee deploy_result.json